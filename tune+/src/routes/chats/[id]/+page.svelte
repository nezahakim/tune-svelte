...
import { MoreVert, Telegram, SkipPrevious} from "$lib/icons";
...
<main class="h-full flex flex-col justify-between">
    <header class="bg-white flex items-center justify-between p-4"> 
        ...
        <div class="flex items-center gap-2">
            <a href="/home">
                {@html SkipPrevious({ width:15, height:15, class: 'text-amber-400' })}
            </a>
            <div class="w-[40pxs] h-[40px] bg-gray-200 p-1 rounded-xl">
            ...
            <div class="flex flex-col">
                ...
                <div class=" flex items-center font-light text-[13px]">
                    <span>online</span>
                    <span class=" flex items-center m-1 w-[5px] h-[5px] text-[6px] bg-green-400 rounded-full text-green-400 "><i>0</i></span>
                </div>
            </div>
         </div>
         ...
         <i>
            {@html MoreVert({ width: 24, height: 24, class: 'text-blue-500' })}
         </i>
    </header>

    <div class="h-full flex flex-col justify-end overflow-y-auto max-h-[calc(100vh-100px)] scroll-smooth overscroll-behavior-y: contain will-change-scroll scrollbar-thin scrollbar-thumb-blue-500 scrollbar-track-gray-200">
         ...
         <div class=" flex flex-col">
            ...
            <div class="flex items-center">
                <hr class="w-5" />
                <span class="text-blue-500 px-2 font-bold text-[10px]">Neza</span>
                <hr class="w-full"/>
            </div>
        
             <p class="text-[14px] px-2 py-0">
               Hello, world
             </p>
         </div>    
         <div class=" flex flex-col">
            ...
            <div class="flex items-center">
                <hr class="w-full"  />
                <span class="text-amber-500 px-2 font-bold text-[10px]">Me</span>
                <hr class="w-5" />
            </div>
             <p class=" flex flex-col items-end text-[14px] px-2 py-0">
               Hello, world
             </p>
         </div> 
    </div>

    <form class="bg-gray-50 p-2 flex items-end w-full justify-between">
        <input bind:value={message} class="p-2 w-full h-[40px] outline-none" name="" id="" placeholder="Write a text message..."/>
        <button class="p-2" onclick={handleSend}>
            {@html Telegram({width: 24, height:24, class:'text-blue-500'})}
        </button>
    </form>
</main> -->
...
<script lang='ts'>
  ...
  import { onMount } from 'svelte';
	import type { ChatMessage, Gift } from '$lib/types';
  ...
  let messages = $state<ChatMessage[]>([
      { text: "Hello, how are you?", sender: "neza", time: "10:24 AM", status: "read" },
      { text: "I'm doing great! How about you?", sender: "me", time: "10:26 AM", status: "read" },
      { text: "I'm good too. Do you want to meet for coffee later?", sender: "neza", time: "10:28 AM", status: "read" }
  ]);
      ...
      setTimeout(() => {
          ...
          setTimeout(() => {
              ...
              setTimeout(() => {
                  isTyping = false;
                  messages = [...messages, {
                      text: `Thank you for the amazing ${gift.name} gift!`,
                      sender: 'neza',
                      time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
                      status: 'sent'
                  }];
  ...
  // Effect to auto-scroll to bottom on mount and when messages change
  $effect(() => {
      scrollToBottom();
  });
  ...
  <main class="h-full flex flex-col justify-between bg-gray-100 relative">
      ...
      >
          ...
          {#each messages as msg, i}
              ...
              <div
                  class="flex flex-col mb-2 max-w-[80%] {msg.sender === 'me' ? 'ml-auto' : 'mr-auto'}"
                  in:fly={{ y: 20, duration: 300 }}
              >
                  <!-- Sender indicator if needed -->
                  {#if i === 0 || messages[i-1].sender !== msg.sender}
                      <div class="flex items-center {msg.sender === 'me' ? 'justify-end' : 'justify-start'}">
                          <hr class="border-gray-400 {msg.sender === 'me' ? 'flex-grow' : 'w-5'}" />
                          <span class="{msg.sender === 'me' ? 'text-amber-500' : 'text-blue-500'} px-2 font-bold text-[10px]">
                              {msg.sender === 'me' ? 'Me' : 'Neza'}
                          </span>
                          <hr class="border-gray-400 {msg.sender === 'me' ? 'w-5' : 'flex-grow'}" />
                      </div>
                  {/if}
  
                  <!-- Message bubble -->
                  <div 
                      class="px-3 py-2 rounded-lg {msg.sender === 'me' ? 
                  ...
                  <!-- Message info -->
                  <div class="flex items-center mt-1 {msg.sender === 'me' ? 'justify-end' : 'justify-start'} text-[10px] text-gray-500">
                      <span>{msg.time}</span>
                      {#if msg.sender === 'me'}
                          <span class="ml-1">
                              {#if msg.status === 'sent'}
          ...
          >
              ...
              <div class="flex border-b border-gray-200 overflow-x-auto scrollbar-thin bg-gray-50">
                  {#each emojiCategories as category, index}
                      <button 
                          on:click={() => changeEmojiCategory(index)}
      ...
      {#if showGiftPicker}
          ...
          >
              <div class="flex justify-between items-center mb-2 border-b pb-2">
                  ...
                  <button on:click={toggleGiftPicker} class="text-gray-500 hover:text-gray-700">
      ...
      <!-- Input area -->
      <form on:submit={handleSend} class="bg-white p-2 flex items-center gap-2 border-t border-gray-200">
          <!-- Gift button -->
          <button 
              type="button" 
              on:click={toggleGiftPicker}
